...
def build_campaign_name(sku: str, type_code: str, match_type: str,
    ...
    - Keyword (Exact): {SKU}_SP00_{SPRxx}_{Keyword}_{Matchtype}_Nguyen_{DDMMYY}
    - Keyword (Khác): {SKU}_SP00_{Keyword}_{Matchtype}_Nguyen_{DDMMYY}
    - Auto: {SKU}_SP02_AUTO_Nguyen_{DDMMYY}
    - PT: {SKU}_SP03_PT_{Target}_Nguyen_{DDMMYY}
    """
    ...
    if is_auto:
        components = [sku, "SP02", "AUTO", "Nguyen", current_date]
    elif is_pt:
        clean_keyword = re.sub(r'[^\w\s-]', '', raw_keyword).strip()
        pt_string = f"PT_{clean_keyword}"
        components = [sku, "SP03", pt_string, "Nguyen", current_date]
    else:
        ...
        if mt == "exact":
            ...
            components = [sku, "SP00", spr_final, keyword_slug, mt_capitalized, "Nguyen", current_date]
        else:
            components = [sku, "SP00", keyword_slug, mt_capitalized, "Nguyen", current_date]
